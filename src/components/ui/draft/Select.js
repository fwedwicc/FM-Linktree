      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="space-y-0.5 absolute w-full mt-[40px] bg-neutral-900 border border-neutral-800/60 rounded-[12px] p-1 z-10"
          >
            {options.map((option) => (
              <button
                key={option}
              ...
              ${value === option ? "bg-neutral-800/60" : "hover:bg-neutral-800/60"}`}
              >
                {option}
              </button>
            ))}
